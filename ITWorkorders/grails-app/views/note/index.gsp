		...
		<div id="list-note" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="date" title="${message(code: 'note.date.label', default: 'Date')}" />
						...
						<g:sortableColumn property="user" title="${message(code: 'note.user.label', default: 'User')}" />
				...
				<g:each in="${noteInstanceList}" status="i" var="noteInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${noteInstance.id}">${fieldValue(bean: noteInstance, field: "date")}</g:link></td>
