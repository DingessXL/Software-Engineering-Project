		...
		<div id="list-priority" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="level" title="${message(code: 'priority.level.label', default: 'Level')}" />
					
						<g:sortableColumn property="name" title="${message(code: 'priority.name.label', default: 'Name')}" />
				...
				<g:each in="${priorityInstanceList}" status="i" var="priorityInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${priorityInstance.id}">${fieldValue(bean: priorityInstance, field: "level")}</g:link></td>
					
						<td>${fieldValue(bean: priorityInstance, field: "name")}</td>
